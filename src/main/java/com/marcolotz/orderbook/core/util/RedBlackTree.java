...
// Reference: https://github.com/Arsenalist/Red-Black-Tree-Java-Implementation
// Modified to used comparators instead of comparable and to extern the Node elements.

// The core code was not implemented by me (Marco Lotz)
/*
...
Since we are only concerned with deleting the value from the tree, we will begin
our delete(RedBlackNode v) function with a call to search(v.value) which will
...
the argument is nil or not.  I have chosen my search(int value) function to be
...
counterpart with the same value.  The insert() function takes care of this
...
public class RedBlackTree<T> {
    ...
    public RedBlackNode<T> insert(T value) {
        return insert(new RedBlackNode<T>(value));
    }
    ...
    private RedBlackNode<T> insert(RedBlackNode<T> z) {
        ...
        while (!isNil(x)) {
            ...
            // if z.value is < than the current value, go left
            ...
            // else z.value >= x.value so go right.
            else {
        ...
        // Depending on the value of y.value, put z as the left or
        // right child of y
        if (isNil(y))
            root = z;
    ...
    // @param: node, the node with the smallest value rooted at node
    public RedBlackNode<T> treeMinimum(RedBlackNode<T> node) {

        // while there is a smaller value, keep going left
    ...
    // @return: return's the node the with the next largest value
    // from x.value
    ...
    // @param: v, the value V which is to be removed from the the tree
    ...
    public void remove(T v) {
        ...
        // updating due to the deletion of z.value.
        fixNodeData(x, y);
    ...
    // @param: value, the value of the value that used to be in y
    private void fixNodeData(RedBlackNode<T> x, RedBlackNode<T> y) {
        ...
        while (!isNil(current)) {
            ...
            if (y.value != current.value) {
                ...
                // current.value then decrement current.numRight
                if (comparator.compare(y.value, current.value) > 0)
                    current.numRight--;

                // if the node we deleted is less than
                // current.value thendecrement current.numLeft
                if (comparator.compare(y.value, current.value) < 0)
                    current.numLeft--;
            }

            // if the node we deleted has the same value as the
    ...
    // @param: value, the value whose node we want to search for
    // @return: returns a node with the value, value, if not found, returns null
    // Searches for a node with value k and returns the first such node, if no
    ...
    // @param: value, any Comparable object
    // @return: return's the number of elements greater than value
    public int numGreater(T value) {

        // Call findNumGreater(root, value) which will return the number
        // of nodes whose value is greater than value
        return findNumGreater(root, value);

    }// end numGreater(int value)


    // @param: value, any Comparable object
    // @return: return's teh number of elements smaller than value
    public int numSmaller(T value) {

        // Call findNumSmaller(root,value) which will return
        // the number of nodes whose value is greater than value
        return findNumSmaller(root, value);

    }// end numSmaller(int value)


    // @param: node, the root of the tree, the value who we must
    // compare other node value's to.
    // @return: the number of nodes greater than value.
    public int findNumGreater(RedBlackNode<T> node, T value) {

        // Base Case: if node is nil, return 0
        if (isNil(node))
            return 0;
            // If value is less than node.value, all elements right of node are
            // greater than value, add this to our total and look to the left
        else if (comparator.compare(value, node.value) < 0)
            return 1 + node.numRight + findNumGreater(node.left, value);

            // If value is greater than node.value, then look to the right as
            // all elements to the left of node are smaller than value
        else
            return findNumGreater(node.right, value);

    }// end findNumGreater(RedBlackNode, int value)

    /**
     * Returns sorted list of values greater than value.  Size of list
     * will not exceed maxReturned
     *
     * @param value       value to search for
     * @param maxReturned Maximum number of results to return
     * @return List of values greater than value.  List may not exceed maxReturned
     */
    public List<T> getGreaterThan(T value, Integer maxReturned) {
        List<T> list = new ArrayList<T>();
        getGreaterThan(root, value, list);
        return list.subList(0, Math.min(maxReturned, list.size()));
    }


    private void getGreaterThan(RedBlackNode<T> node, T value,
                                List<T> list) {
        if (isNil(node)) {
            return;
        } else if (comparator.compare(node.value, value) > 0) {
            getGreaterThan(node.left, value, list);
            list.add(node.value);
            getGreaterThan(node.right, value, list);
        } else {
            getGreaterThan(node.right, value, list);
        }
    }

    // @param: node, the root of the tree, the value who we must compare other
    // node value's to.
    // @return: the number of nodes smaller than value.
    public int findNumSmaller(RedBlackNode<T> node, T value) {

        // Base Case: if node is nil, return 0
        if (isNil(node)) return 0;

            // If value is less than node.value, look to the left as all
            // elements on the right of node are greater than value
        else if (comparator.compare(value, node.value) <= 0)
            return findNumSmaller(node.left, value);

            // If value is larger than node.value, all elements to the left of
            // node are smaller than value, add this to our total and look
            // to the right.
        else
            return 1 + node.numLeft + findNumSmaller(node.right, value);

    }// end findNumSmaller(RedBlackNode nod, int value)
...
RedBlackNode holds a Comparable "value", which is the value of the RedBlackNode.  It
...
instantiated.
The constructor that takes in a Comparable argument assigns that value to the value
of the node. The empty constructor is there to test Prof. Pitt's test case and
also in case we want to just create a RedBlackNode and initialize its value later.
